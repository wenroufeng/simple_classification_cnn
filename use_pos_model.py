...
from train_pos import Residual
...
model = torch.load("pos_models/model_10.pth", map_location=torch.device(device))
...
with torch.no_grad():
    ...
    for sample in dataset.iter_samples(autosave=True):
        ...
        sample['位置'] = fo.Classification(label=v_c_map[output.argmax(1).tolist()[0]], confidence=output.softmax(dim=1).flatten().tolist()[1])
